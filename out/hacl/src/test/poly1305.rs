...
pub fn test_poly1305() {
  let mut tag = [ 0u8; 16];
  crate::hacl::mac_poly1305::Hacl_MAC_Poly1305_mac(&mut tag, &input1, input1.len() as u32, &key1);
  assert_eq!(tag, tag1);
  crate::hacl::mac_poly1305::Hacl_MAC_Poly1305_mac(&mut tag, &input2, input2.len() as u32, &key2);
  assert_eq!(tag, tag2);
  crate::hacl::mac_poly1305::Hacl_MAC_Poly1305_mac(&mut tag, &input3, input3.len() as u32, &key3);
  assert_eq!(tag, tag3);
}
