...
pub fn test_p256() {
  ...
  let valid = crate::hacl::p256::Hacl_P256_dh_initiator(&mut pk, &mut private_key);
  assert!(valid);
  assert_eq!(pk, public_key);

  let flag = crate::hacl::p256::Hacl_P256_ecdsa_sign_p256_sha2(&mut sig, 128, &mut msg, &mut private_key, &mut nonce);
  assert!(flag);
  assert_eq!(sig, sgnt);
  ...
  let ver = crate::hacl::p256::Hacl_P256_ecdsa_verif_p256_sha2(128, &mut msg, &mut public_key, &mut sp.0, &mut sp.1);
  assert!(ver);
}
