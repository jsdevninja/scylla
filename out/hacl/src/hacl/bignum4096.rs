...
)
{
  if bBits < 200u32
  {
    ...
    ((&mut (&mut ctx)[64usize..])[0usize..64usize]).copy_from_slice(&r2[0usize..64usize]);
  ...
  for i in 0u32..7u32
  {
    ...
    ((&mut aM_copy0)[0usize..64usize]).copy_from_slice(&t11.1[0usize..64usize]);
    let ctx_n1: (&[u64], &[u64]) = ctx.split_at(0usize);
    amont_sqr(ctx_n1.1, mu, &aM_copy0, &mut tmp);
    crate::lowstar::ignore::ignore::<&[u64]>(&ctx);
    ((&mut (&mut table)[2u32.wrapping_mul(i).wrapping_add(2u32).wrapping_mul(64u32) as usize..])[0usize..64usize]).copy_from_slice(
      &(&tmp)[0usize..64usize]
    );
    let t2: (&[u64], &[u64]) =
    ...
    crate::lowstar::ignore::ignore::<&[u64]>(&ctx);
    ((&mut (&mut table)[2u32.wrapping_mul(i).wrapping_add(3u32).wrapping_mul(64u32) as usize..])[0usize..64usize]).copy_from_slice(
      &(&tmp)[0usize..64usize]
    )
  };
  if bBits.wrapping_rem(4u32) != 0u32
  {
    ...
    ((&mut resM)[0usize..64usize]).copy_from_slice(&(a_bits_l.1 as &[u64])[0usize..64usize])
  }
  else
  {
    let ctx_n: (&[u64], &[u64]) = ctx.split_at(0usize);
  ...
  for i in 0u32..bBits.wrapping_div(4u32)
  {
    ...
    let a_bits_l: (&[u64], &[u64]) = table.split_at(bits_l32.wrapping_mul(64u32) as usize);
    ((&mut tmp0)[0usize..64usize]).copy_from_slice(&(a_bits_l.1 as &[u64])[0usize..64usize]);
    let mut aM_copy: [u64; 64] = [0u64; 64usize];
    ((&mut aM_copy)[0usize..64usize]).copy_from_slice(&(&resM)[0usize..64usize]);
    let ctx_n: (&[u64], &[u64]) = ctx.split_at(0usize);
...
)
{
  ...
  if bBits.wrapping_rem(4u32) != 0u32
  {
    ...
    ((&mut resM)[0usize..64usize]).copy_from_slice(&(&table as &[u64])[0usize..64usize]);
  ...
  for i0 in 0u32..bBits.wrapping_div(4u32)
  {
    ...
    let bits_l: u64 = crate::hacl::bignum_base::Hacl_Bignum_Lib_bn_get_bits_u64(bLen, b, k, 4u32);
    crate::lowstar::ignore::ignore::<&[u64]>(&table);
    ((&mut tmp0)[0usize..64usize]).copy_from_slice(&(&table as &[u64])[0usize..64usize]);
    for i1 in 0u32..15u32
    {
