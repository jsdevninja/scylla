...
let adjust e t =
  ...
  | _, _, TUnit when e.typ <> TUnit ->
      with_type TUnit (EIgnore e)
  (* TODO: tag indices *)
  | _ ->
      if e.typ <> t then
...
let mk_binop lhs kind rhs =
  ...
  | _, EQ when rhs.node = EBufNull ->
      Helpers.efalse
  | _, Comma ->
      with_type rhs.typ (ESequence [ adjust lhs TUnit; rhs ])
...
let translate_external_fundecl ?(flags=[]) (fdecl : function_decl) =
  let name = get_id_name fdecl.name in
  ...
  Krml.Ast.(
    DExternal (None, flags, 0, 0, lid, fn_type, List.map (fun x -> Krml.Ast.(x.node.name)) binders))
...
let translate_external_decl ignored_dirs (decl : decl) =
  decl_error_handler ~ignored_dirs decl None @@ fun () ->
  match decl.desc with
  | Function fdecl ->
      (* let name = get_id_name fdecl.name in *)
      if Attributes.has_opaque_attr fdecl.attributes then
        ...
        Some (translate_external_fundecl ~flags:[Private] fdecl)
  | _ -> None
