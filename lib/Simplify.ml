  ...
  object (self)
    ...
    *)
    method! visit_ELet _ b e1 e2 =
      ...
      | EAny, EAssign (var, e) when var.node = EBound 0 -> ELet (b, e, Krml.Helpers.eunit)
      | _ -> ELet (b, e1, e2)
  end
  ...
  object (_self)
    ...
    method! visit_ECast (((), _) as env) e t_to =
      ...
      | ( (TArray (TInt w_from, _) | TBuf (TInt w_from, _)),
          (TArray (TInt w_to, _) | TBuf (TInt w_to, _)) )
        when w_from <> w_to ->
          let is_const =
            match t_to with
            | TBuf (_, false) -> false
          ...
          let name =
            Printf.sprintf "scylla_%s_of_%s%s"
              (PrintMiniRust.string_of_width w_to)
              (PrintMiniRust.string_of_width w_from)
              (if is_mut then
                 "_mut"
               else
                 "")
          in
          let scylla_cast =
            with_type (TArrow (t_from, t_to)) (EQualified ([ "scylla_glue" ], name))
          in
          EApp (scylla_cast, [ e ])
      | _ -> super#visit_ECast env e t_to
  end
