...
let retrieve_empty_variants (attrs : attribute list) = List.filter_map retrieve_empty_variant attrs
...
let retrieve_mutability' (attr : attribute) =
  ...
  | Clang__.Attributes.Annotate s ->
      ...
      if String.starts_with ~prefix:mut_attr s.annotation then
        ...
        let ret =
          String.trim
            (String.sub s.annotation (close_paren + 1)
               (String.length s.annotation - (close_paren + 1)))
        in
        let ret =
          if ret <> "" then
...
let retrieve_alignment (attrs : attribute list) =
  List.find_map
    (fun (x : attribute) ->
      match x.desc with
      | Clang__.Attributes.Aligned { alignment_expr; _ } -> begin
          match alignment_expr with
          | { desc = IntegerLiteral n; _ } -> Some (Clang.Ast.int_of_literal n)
          | _ ->
...
let has_always_inline (attrs : attribute list) =
  List.exists
    (fun (x : attribute) ->
      match x.desc with
      | Clang__.Attributes.AlwaysInline _ -> true
      | _ -> false)
    attrs
...
(* This attempts to read the attributes since typedef attributes are not exposed in the
   ClangMl high-level AST. This is painful. *)
let decl_is_opaque (decl : decl) = decl_has_attr decl opaque_attr
let decl_has_default decl = decl_has_attr decl default_attr
let decl_is_container decl = decl_has_attr decl container_attr
